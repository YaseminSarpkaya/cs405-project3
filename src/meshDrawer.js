...
void main()
{
	...
	//taken from cs405 week13 recitations

	float diffLight = max(dot(normal, lightdir), 0.0); // Simple diffuse lighting
	diff = diffLight;

	
	vec3 viewDir = normalize(-normal); // We are looking along the z axis
	vec3 reflectDir = reflect(-lightdir, normal); // Compute reflection vector
	float specVw = max(dot(reflectDir, viewDir), 0.0);
	spec = pow(specVw, phongExp);
